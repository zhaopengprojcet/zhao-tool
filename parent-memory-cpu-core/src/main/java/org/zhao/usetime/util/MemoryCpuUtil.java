...
public class MemoryCpuUtil {
	...
	private static Log logger = LogFactory.getLog(MemoryCpuUtil.class);
	...
	public static List<Double> getMemoryCupData() {
		com.sun.management.OperatingSystemMXBean mxbean =
				 
			      (com.sun.management.OperatingSystemMXBean)
			      ManagementFactory.getOperatingSystemMXBean();
		List<Double> datas = new ArrayList<Double>();
		double precent = mxbean.getTotalPhysicalMemorySize()*1.0;
		datas.add(precent);//总内存
		
		precent = mxbean.getFreePhysicalMemorySize()*1.0;
		datas.add( precent);//可用内存
		
		datas.add(getCpuUsage());
		
		try {
			datas.add(getDiskUsage());
		} catch (Exception e) {
			datas.add(-1D);
			e.printStackTrace();
		}
		...
		if (osName.toLowerCase().contains("windows")
			...
			try {
				...
				if (c0 != null && c1 != null) {
					...
					Double cpusage = Double.valueOf(PERCENT * (busytime) * 1.0
							/ (busytime + idletime));
					BigDecimal b1 = new BigDecimal(cpusage);
					double cpuUsage = b1.setScale(2, BigDecimal.ROUND_HALF_UP)
							.doubleValue();
					return cpuUsage;
				} else {
					return 0.0;
			...
			try {
				Map<?, ?> map1 = MemoryCpuUtil.cpuinfo();
				Thread.sleep(CPUTIME);
				Map<?, ?> map2 = MemoryCpuUtil.cpuinfo();
